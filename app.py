...
st.markdown("""
    ...
    .kpi-box {
        ...
        border: 2px solid #ddd; height: 180px; display: flex; flex-direction: column; justify-content: center;
    }
    .kpi-label { margin:0; font-size: 22px !important; font-weight: bold; color: #1E1E1E; }
    .kpi-value { margin:0; font-size: 34px !important; font-weight: bold; }
...
ETTARI_FILIERA, LOSS_SOC_BASE_HA = 12000, 0.5
BASELINE_TOT_ANNUA = ETTARI_FILIERA * (4.5 + LOSS_SOC_BASE_HA)

def run_scaling_sim():
    anni, results_ha, budgets, traiettoria = [2026, 2027, 2028, 2029, 2030], [], [], [BASELINE_TOT_ANNUA]
    imp_val = ((-df_p['d_emiss'] + df_p['d_carb'] + LOSS_SOC_BASE_HA) * 0.9)
    stock_acc, total_co2 = 0, 0
    for i, anno in enumerate(anni):
        bt = budget_iniziale * ((1 + crescita_budget_pct/100) ** i)
        budgets.append(bt)
        ha = {p: (bt * (st.session_state[k]/100)) / df_p.at[p, 'costo'] for p, k in zip(df_p.index, ['cover', 'inter', 'comb'])}
        tot_ha = sum(ha.values())
        if tot_ha > ETTARI_FILIERA:
            for p in ha: ha[p] *= (ETTARI_FILIERA / tot_ha)
        stock_acc = (stock_acc * 0.9 * 0.75) + sum(ha[p] * imp_val[p] for p in ha)
        traiettoria.append(BASELINE_TOT_ANNUA - stock_acc)
        total_co2 += stock_acc
        results_ha.append(ha.copy())
    return anni, traiettoria, results_ha, budgets, total_co2
...
c2.markdown(f'<div class="kpi-box"><p class="kpi-label">€/t CO2</p><p class="kpi-value" style="color:#1a73e8;">{investimento_totale/co2_totale:.2f}</p></div>', unsafe_allow_html=True)
c3.markdown(f'<div class="kpi-box"><p class="kpi-label">Budget 5Y</p><p class="kpi-value">€ {int(investimento_totale/1000)}k</p></div>', unsafe_allow_html=True)
...
c5.markdown(f'<div class="kpi-box" style="border: 3px solid {col_gap};"><p class="kpi-label">Gap Target</p><p class="kpi-value" style="color:{col_gap};">{int(gap_2030)} t</p></div>', unsafe_allow_html=True)
...
with l:
    ...
    fig.add_trace(go.Scatter(x=[2025, 2030], y=[BASELINE_TOT_ANNUA*(1-target_decarb_req/100)]*2, line=dict(dash='dash', color='red', width=3), name="Target FLAG"))
    
    # FIX: Sintassi Legend e Font separata per evitare l'errore
    fig.update_layout(
        height=500,
        margin=dict(t=50),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(size=CHART_FONT_SIZE)),
        xaxis=dict(title="Anno", tickfont=dict(size=CHART_FONT_SIZE), titlefont=dict(size=CHART_FONT_SIZE+2)),
        yaxis=dict(title="t CO2eq", tickfont=dict(size=CHART_FONT_SIZE), titlefont=dict(size=CHART_FONT_SIZE+2), tickformat=",.0f")
    )
    st.plotly_chart(fig, use_container_width=True)

with r:
    st.subheader("📊 Mix Ettari 2030")
