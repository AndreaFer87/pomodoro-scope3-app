...
st.markdown("""
    ...
    section[data-testid="stSidebar"] div[data-testid="stWidgetLabel"] p { font-size: 20px !important; font-weight: bold !important; color: #000 !important; }
    section[data-testid="stSidebar"] .stMarkdown h2 { font-size: 26px !important; color: #000 !important; border-bottom: 2px solid #2E7D32; }
    </style>
...
# --- MOTORE DI SIMULAZIONE (IL TUO BLOCCO LOGICO) ---
def run_matrix_sim():
    ...
    for i, anno in enumerate(anni):
        ...
        for prov, data in DB_GEO.items():
            for pratica, tasso in data['ado'].items():
                ...
                # --- ESTREMA ATTENZIONE: IL TUO BLOCCO RICHIESTO ---
...
c2.markdown(f'<div class="kpi-box"><p class="kpi-label">ROI Climatico</p><p class="kpi-value" style="color:#1a73e8;">{sum(budgets)/co2_totale:.2f} €/t</p></div>', unsafe_allow_html=True)
c3.markdown(f'<div class="kpi-box"><p class="kpi-label">Investimento 5Y</p><p class="kpi-value">€ {int(sum(budgets)):,}</p></div>', unsafe_allow_html=True)
...
with l:
    ...
    fig.update_layout(height=450, yaxis=dict(tickformat=",.0f", range=[20000, 65000]))
    st.plotly_chart(fig, use_container_width=True)
with r:
    st.subheader("🚜 Mix Pratiche (ha)")
    ...
    fig_bar.update_layout(barmode='stack', height=450)
