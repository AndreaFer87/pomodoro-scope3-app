...
w_diff = st.sidebar.slider("Peso Facilità (Difficoltà)", 0.0, 1.0, 0.2)
...
prob_minima = st.sidebar.slider("Adozione Probabilistica Minima (%)", 0, 20, 10)
...
# --- FUNZIONE STANDARDIZZAZIONE SICURA ---
...
# --- CALCOLI ---
df_p['Imp_Val'] = ((-df_p['d_emiss'] + df_p['d_carb'] + LOSS_SOC_BASE_HA) * (1 - safety_buffer/100)).round(3)
...
if not pratiche_facili.empty:
    ha_per_pratica = (ETTARI_FILIERA * (prob_minima/100)) / len(pratiche_facili)
    for p in pratiche_facili:
        ettari_allocati[p] = ha_per_pratica
...
c4.metric("Gap Target", f"{int(max(0, target_ton - abb_tot))} t")
...
with col_l:
    ...
    if values:
        ...
        st.error("Budget insufficiente per la quota minima!")

with col_r:
    st.subheader("📈 Ranking Score")
    fig_bar = go.Figure(go.Bar(x=df_p['Score'], y=df_p.index, orientation='h', marker_color='lightseagreen'))
    fig_bar.update_layout(xaxis=dict(range=[0, 1]), yaxis={'categoryorder':'total ascending'})
